...
namespace FM\SymSlateBundle\PO;
...
class PoFile
{
	...
	private static function unformat($string)
	{
		return str_replace('\n', "\n", str_replace('\"', '"', $string));
	}
	...
	public function addMessage($message, $translation, $context = "")
	{
		...
		//$different = isset($this->entries[$msgid]) && $this->entries[$msgid]['msgstr']
			...
			if($line == "")
			{
				if($state != self::ParserInWhiteSpace) // do nothing if this is not the first whitespace line we encounter
				{
					...
					if(isset($current_entry_data['msgid']))
					{
						...
						$unescaped_msg_id = static::unformat($msgid);

						if(isset($hash[$unescaped_msg_id]))
						{
							throw new Exception("Duplicate entry found for msgid '{$current_entry_data['msgid']}' (at line: $line_number)");
						}
						else
						{
							$hash[$unescaped_msg_id] = $current_entry_data;
	...
	public function getEntry($message, $context=null)
	{
		$msgid = $message;
		if($context !== null)
		{
